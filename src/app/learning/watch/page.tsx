...
export default async function WatchPage() {
  ...
  return <main className="page-shell"><Breadcrumb items={[{ label: "Home", href: "/learning" }, { label: "Watch" }]} /><PageHero eyebrow="Watch" title="Xem và khám phá" lead="Cùng con lắng nghe tiếng Anh qua những câu chuyện và hội thoại sinh động." /><section className="choice-grid">{types.map((item) => <CategoryCard key={item.id} href={CONTENT_TYPE_PATHS[`watch/${item.slug}`]} name={item.name} description={item.description} illustrationUrl={item.illustrationUrl} />)}</section></main>;
}
