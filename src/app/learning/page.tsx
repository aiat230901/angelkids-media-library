...
import { CATEGORY_PATHS } from "@/config/learning-routes";
...
export default async function LearningPage() {
  ...
  return <main className="page-shell"><PageHero eyebrow="Angel Kids Learning Hub" title="Hôm nay con muốn khám phá gì?" lead="Một không gian học liệu vui tươi để con xem, đọc, nghe và ôn tập mỗi ngày." />
    ...
    <section className="category-grid" aria-label="Danh mục học liệu">{categories.map((category) => <CategoryCard key={category.id} href={CATEGORY_PATHS[category.slug] ?? "/learning"} name={category.name} description={category.description} illustrationUrl={category.illustrationUrl} featured={category.slug === "print-and-plays"} />)}</section>
  </main>;
}
