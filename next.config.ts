...
const heyzineHostnames = (process.env.HEYZINE_ALLOWED_HOSTS ?? "heyzine.com,mamnonangelkids.aflip.in").split(",").map((host) => host.trim()).filter(Boolean);
const heyzineHosts = heyzineHostnames.map((host) => `https://${host}`).join(" ");
const thumbnailHostnames = (process.env.THUMBNAIL_ALLOWED_HOSTS ?? "i.ytimg.com,cdnm.heyzine.com").split(",").map((host) => host.trim()).filter(Boolean);
